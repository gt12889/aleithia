    ...
    <div className="border border-white/[0.06] bg-white/[0.02] p-5">
      ...
      <div className="flex items-end gap-px h-24">
        {data.hours.map(bucket => {
          ...
          const color =
            bucket.density === 'high' ? 'bg-green-500/70' :
            bucket.density === 'medium' ? 'bg-amber-500/70' :
            'bg-white/10'
          ...
          return (
            ...
            >
              ...
              <div className="absolute bottom-full mb-1 hidden group-hover:block z-10 pointer-events-none">
                <div className="bg-black/90 border border-white/10 px-2 py-1 text-[9px] font-mono text-white/70 whitespace-nowrap">
                  {formatHour(bucket.hour)}: {bucket.avg_vehicles} veh, {bucket.avg_pedestrians} ped (IDOT hwy)
