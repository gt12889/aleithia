...
# When vectordb_image is None (default), skip Modal class/function registration.
# All callers use vectordb_available() + try/except for graceful degradation.
# Set VECTORDB_IMAGE=1 env var to enable the VectorDB service.
if vectordb_image is not None:
    ...
    class VectorDBService:
        ...
        def upsert_doc(self, doc_id: str, embedding: list[float], payload: dict, collection: str = "enriched"):
            ...
            span = span_ctx.__enter__() if span_ctx else None
            try:
                self._client.upsert(collection=collection, id=doc_id, vector=embedding, payload=payload)
                if span:
        ...
        def batch_upsert_docs(self, doc_ids: list[str], embeddings: list[list[float]], payloads: list[dict], collection: str = "enriched"):
            ...
            span = span_ctx.__enter__() if span_ctx else None
            try:
                self._client.batch_upsert(collection=collection, ids=doc_ids, vectors=embeddings, payloads=payloads)
        ...
        def search_neighborhood(self, query_embedding: list[float], neighborhood: str, top_k: int = 50) -> list[dict]:
            ...
            return self.search.local(
                query_embedding=query_embedding, collection="enriched",
                top_k=top_k, filter_dict={"neighborhood": neighborhood},
            )
