...
# The project has a local `modal/` directory that can shadow the installed
# `modal` pip package. Temporarily adjust sys.path so we import the real one.
_project_root = str(pathlib.Path(__file__).resolve().parent.parent)


def _import_modal():
    """Import the installed modal package, not any local modal/ directory."""
    _removed = []
    for p in sys.path[:]:
        try:
...
def run(query: str, max_videos: int = MAX_VIDEOS_PER_QUERY, verbose: bool = False) -> None:
    ...
    # Step 2: Scrape TikTok (parallel via Modal .starmap)
    console.print("[bold]Scraping TikTok...[/bold]")
    try:
        scrape_fn = Function.from_name("alethia", "scrape_tiktok")
    except Exception as exc:
        console.print(
            f"[red]Could not find deployed scrape function:[/red] {exc}\n"
            "  Run: modal deploy modal_app/pipelines/tiktok.py"
        )
        sys.exit(1)
    ...
    try:
        transcribe_fn = Function.from_name("alethia", "transcribe_video")
    except Exception as exc:
        console.print(
